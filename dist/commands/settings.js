...
async function commandSettings(ctx) {
...
async function showWalletInfo(ctx) {
    const userId = ctx.from?.id.toString();
    if (!userId)
        return;
    const user = await prisma_1.prisma.user.findUnique({
...
async function showSendPayment(ctx) {
    const userId = ctx.from?.id.toString();
    if (!userId)
        return;
    const user = await prisma_1.prisma.user.findUnique({
...
async function showReceivePayment(ctx) {
    const userId = ctx.from?.id.toString();
    if (!userId)
        return;
    const user = await prisma_1.prisma.user.findUnique({
...
async function showExportKey(ctx) {
    const userId = ctx.from?.id.toString();
    if (!userId)
        return;
    const user = await prisma_1.prisma.user.findUnique({
    ...
    try {
        ...
        const { decryptPrivateKey } = await Promise.resolve().then(() => require("../core/wallets"));
