...
exports.handleTipConfirmation = handleTipConfirmation;
...
async function commandTip(ctx) {
    ...
    const u = ctx.update;
    console.log('TIP RAW:', util_1.default.inspect(u, { depth: 6, colors: true }));
    ...
    logger_1.logger.info("Tip command received");
    ...
    if (!target && explicitMention) {
        ...
        if (userId)
            target = { id: userId, username: explicitMention.slice(1) };
    }
    if (!target) {
    ...
    try {
        ...
        const chatRecord = isGroupChat ? await prisma_1.prisma.chat.findUnique({
        ...
        const token = await (0, tokens_1.resolveToken)(finalTokenTicker);
        if (!token) {
        ...
        const tipper = await prisma_1.prisma.user.findUnique({
        ...
        if (payeeHandle) {
            ...
            const payeeResult = await prisma_1.prisma.user.findFirst({
                where: {
                    handle: {
                        equals: payeeHandle,
            ...
            const payeeResult = await prisma_1.prisma.user.findUnique({
        ...
        const feeMessage = message_templates_1.messages.dm.tip_confirmation(messageData);
        // Generate payment ID
        const paymentId = (0, uuid_1.v4)();
        const clientIntentId = (0, idempotency_1.generateClientIntentId)(tipperId, paymentId);
        ...
        const { InlineKeyboard } = await Promise.resolve().then(() => require("grammy"));
        ...
        logger_1.logger.info(`Tip confirmation requested: ${paymentId}`);
    }
    catch (error) {
        logger_1.logger.error("Tip command error:", error);
        await ctx.reply("❌ Tip failed. Please try again.");
    }
}
async function handleTipConfirmation(ctx, confirmed) {
    try {
        ...
        // Get wallet info
        const tipperWallet = await prisma_1.prisma.wallet.findFirst({
            where: {
        ...
        if (result.success) {
            ...
            const tipSuccessMessage = message_templates_1.messages.dm.tip_sent_confirmation(tipConfirmData);
            ...
            if (payment.to?.telegramId && payment.from?.handle && result.signature) {
                try {
                    ...
                    logger_1.logger.info("Tip notification sent to recipient");
                }
                catch (notificationError) {
                    logger_1.logger.error("Failed to send tip notification", notificationError);
                    // Don't fail the tip if notification fails
                }
            }
            logger_1.logger.info(`Tip completed: ${paymentId}, tx: ${result.signature}`);
        }
        else {
        ...
        logger_1.logger.error("Tip confirmation error:", error);
